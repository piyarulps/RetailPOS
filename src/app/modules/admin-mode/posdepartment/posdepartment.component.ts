...
export interface PeriodicElement {
  ...
  ItemSellingRule: string
}

@Component({
  selector: 'app-posdepartment',
  templateUrl: './posdepartment.component.html',
  styleUrls: ['./posdepartment.component.scss']
})
export class PosdepartmentComponent implements OnInit, AfterViewInit {
  moduleName: string = 'POSDepartment';
  displayModuleName: string = 'POS Department';
  moduleAPI: string = 'itemapis/posdepartment/';
  moduleFilterAPI: string = 'itemapis/filters/';
  moduleLink: string = 'admin-mode/posdepartment';
  displayedColumns: any = ['select', 'POSStatus', 'POSDepartmentName', 'ParentPOSDepartment', 'Description', 'Number', 'ItemSellingRule'];
  dataList: any[];
  ...
  delete() {
    ...
    this._helper.confirmDialog({ message: this.deleteMessage }).subscribe(res => {
      if (res) {
        ...
        this._helper.apiRequest('delete', 'itemapis/posdepartment/delete/', { body: deleteData }).subscribe(res => {
          // console.log("res", res);
          if (res.status == 2) {
    ...
    this._helper.confirmDialog({ message: this.statusMessage }).subscribe(res => {
      if (res) {
        ...
        statusData.forEach((value, key) => {
          statusData[key].posDepStatus = status;
        });
        // console.log('statusData', statusData);
        this._helper.apiPost(statusData, 'itemapis/setPosDepStatus/').subscribe(
          res => {
            if (res.message) {
              this._helper.notify({ message: res.message, messageType: res.status });
            }
