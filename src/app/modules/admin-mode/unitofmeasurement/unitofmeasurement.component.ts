...
export class UnitofmeasurementComponent implements OnInit {
  ...
  displayedColumns: any = ['select', 'status', 'Name', 'Description', 'EnglishMetricFlagName', 'MeasurementSystemName', 'UnitOfMeasureCode'];
  dataList: any[];
    ...
    this._helper.confirmDialog({ message: this.statusMessage }).subscribe(res => {
      if (res) {
        ...
        statusData.forEach((value, key) => {
          statusData[key].uomStatus = status;
        });
        this._helper.apiPost(statusData, 'itemapis/setUOMStatus/').subscribe(
          res => {
            if (res.message) {
              this._helper.notify({ message: res.message, messageType: res.status });
            }
