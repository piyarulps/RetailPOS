...
export interface PeriodicElement {
  ...
  name: string,
  description: string
  sappMayNotBeSold: number,
  sappIsNotReturnable: number,
  sappTax1: number,
  sappTax2: number,
  sappTax3: number,
  sappTax4: number,
  appCheckId1: string,
  sappCheckId2: string,
  sappFractionalQuantity: number,
  sappOpen: number,
  sappSpecialDiscountAllowed: number,
  sappBlueLaw1Applies: number,
  sappBlueLaw2Applies: number,
  sappDepartmentSpecialDiscount: number,
  sappLottoTracking: number,
  sappModifyItem: number,
  sappItemNotCachedOnPos: number,
  passMaxAmount: number,
  passModifierQuantity: number,
  passOpen: number,
  passFoodStamp: string,
  passDiscount: number,
  passQuantityNotAllow: number,
  passQuantityRequired: number,
  allowEmployeeDiscount: number,
  allowFoodStamp: number,
  allowGiveAway: number,
  allowWIC: number,
  allowMultipleCoupon: number,
  couponRestricted: number,
  processMode: string,
  electronicsCoupon: number,
  frequentShopperPointEligible: boolean,
  frequentShopperPointCount: number,
  mustVerifyPrice: number,
  quantityKeyActionMode: string,
  repeatKeyProhibited: boolean,
  requiresPriceEntry: boolean,
  requiresWeightEntry: boolean,
  returnProhibited: boolean,
  itemTenderRestrictionGroupName: string,
  passportProduct: number,
  status: boolean,
}

@Component({
  selector: 'app-selling-rule',
  templateUrl: './selling-rule.component.html',
  styleUrls: ['./selling-rule.component.scss']
})
export class SellingRuleComponent implements OnInit, AfterViewInit {
  ...
  displayedColumns: any = ['select', 'status', 'Name', 'Description', 'SellingStatusCode', 'QuantityKeyActionCode', 'ItemSellingRule', 'FrequentShopperPointsCount', 'EmployeeDiscountAllowedFlag', 'AllowFoodStampFlag', 'AllowCouponMultiplyFlag', 'WICFlag', 'GiveawayFlag', 'CouponRestrictedFlag', 'ElectronicCouponFlag', 'FrequentShopperPointsEligibilityFlag', 'VisualVerifyPriceFlag', 'ProhibitReturnFlag', 'PriceEntryRequiredFlag', 'WeightEntryRequiredFlag', 'ProhibitRepeatKeyFlag', 'MaxAmount', 'ModifierQuantity', 'Open', 'FoodStamp', 'Discount', 'QuantityNotAllowed', 'QuantityRequired', 'Tax1', 'Tax2', 'Tax3', 'Tax4', 'CheckId1', 'CheckId2', 'FractionalQuantity', 'OpenSappire', 'SpecialDiscountAllowed', 'DepartmentSpecialDiscount', 'LottoTracking', 'MayNotBeSold', 'IsNotReturnable', 'BlueLaw1Applies', 'BlueLaw2Applies', 'ModifyItem', 'ItemNotCatchedOnPOS'];
  dataList: any[];
    ...
    this._helper.confirmDialog({ message: this.statusMessage }).subscribe(res => {
      if (res) {
        ...
        statusData.forEach((value, key) => {
          statusData[key].type_value = status;
        });
        // console.log('statusData', statusData);
        this._helper.apiPut(statusData, 'itemapis/updatesellingrulestatus/').subscribe(
          res => {
            if (res.message) {
              this._helper.notify({ message: res.message, messageType: res.status });
            }
