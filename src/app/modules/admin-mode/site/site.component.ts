...
export interface PeriodicElement {
  ...
  TimeZoneCode: string,
  SiteTypeCode: string,
  RetailSiteTypeCode: string
  SiteNorthAngleOffsetDegrees: string
}
...
export class SiteComponent implements OnInit {
  ...
  displayedColumns: any = ['select', 'SiteID', 'TimeZoneCode', 'SiteTypeCode', 'RetailSiteTypeCode', 'SiteNorthAngleOffsetDegrees'];
  dataList: any[];
    ...
    this._helper.confirmDialog({ message: this.statusMessage }).subscribe(res => {
      if (res) {
        ...
        statusData.forEach((value, key) => {
          statusData[key].siteStatus = status;
        });
        // console.log('statusData', statusData);
        this._helper.apiPost(statusData, 'itemapis/setbusinessSiteStatus/').subscribe(
          res => {
            if (res.message) {
              this._helper.notify({ message: res.message, messageType: res.status });
            }
