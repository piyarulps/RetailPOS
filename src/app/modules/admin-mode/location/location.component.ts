...
export interface PeriodicElement {
  ...
  locationHierarchyGroupID: string
...
export class LocationComponent implements OnInit {
  ...
  displayedColumns: any = ['select', 'LocationText', 'ParentLocationID', 'LocationHierarchyGroupID', 'LocationTypeCode', 'LocationLevelID', 'SizeUnitOfMeasureCode', 'SizeQuantity', 'SiteID', 'FunctionCode'];
  dataList: any[];
    ...
    this._helper.confirmDialog({ message: this.statusMessage }).subscribe(res => {
      if (res) {
        ...
        statusData.forEach((value, key) => {
          statusData[key].locationStatus = status;
        });
        // console.log('statusData', statusData);
        this._helper.apiPost(statusData, 'itemapis/statuslocation/').subscribe(
          res => {
            if (res.message) {
              this._helper.notify({ message: res.message, messageType: res.status });
            }
