...
app.get("/", function (req, res) {
    ...
    //     crossOrigin: false,
    //     crossDomain: false,
    //     url: appLoader.BASEURL+"organizations/?partyrole=Vendor&vendortype=Supplier&limit=0&IPADDRESS="+ ipinfo,
    //     headers: {
    ...
    var getURL = appLoader.BASEURL + "organizations/?partyrole=Vendor&vendortype=Supplier&limit=0&IPADDRESS=" + ipinfo;
    appLoader.CallDataFromServer("GET", getURL, {}).then(function (response) {
        ...
        if (DataDownloadFromServer.length > 0) {
            ...
            var promise1 = new Promise(function (resolve, reject) {
                ...
                DataDownloadFromServer.forEach(function (key, val) {
                    ...
                    if (typeof (_manufacturerName) != 'undefined') {
                        ...
                        SupplierdbLoader.SupplierListDB.find({ SupplierName: _manufacturerName }, function (err, docs) {
                            if (docs.length === 0) {
                                SupplierdbLoader.SupplierListDB.insert({ "PartyData": ManufacturerManageArr, "ContactMethod": ContactMethod, "SupplierName": _manufacturerName }, function (err) { });
                                tempCount++;
                            } else {
                                if ('VendorID' in docs[0].PartyData) {
                                    SupplierdbLoader.SupplierListDB.update({ _id: docs[0]._id }, { $set: { "PartyData": ManufacturerManageArr, "ContactMethod": ContactMethod, "SupplierName": _manufacturerName } }, function (err) {
                                        if (err) {
                                            errCount += 1;
                                        } else {
                                            console.log('Updated.');
                                    ...
                                    SupplierdbLoader.SupplierListDB.remove({ _id: docs[0]._id }, function (err, cb) {
                                        if (cb === 1) {
                                            console.log('Updated.');
                                            SupplierdbLoader.SupplierListDB.insert({ "PartyData": ManufacturerManageArr, "ContactMethod": ContactMethod, "ManufacturerName": _manufacturerName }, function (err) { });
                                            tempCount++;
                            ...
                            if (TotolCount == tempCount) {
                                resolve(TotolCount);
                                DownloadeddbLoader.DownloadedDB.insert({ 'ModuleName': 'Suppliers', 'Total': TotolCount, 'Downloaded': tempCount });
            ...
            DownloadeddbLoader.DownloadedDB.insert({ 'ModuleName': 'Suppliers', 'Total': 0, 'Downloaded': 0 });
