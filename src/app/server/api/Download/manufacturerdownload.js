...
app.get("/", function (req, res) {
    ...
    var ManufacturerdbLoader = require('../Vendors/vendordbloader.js');

    var getURL = appLoader.BASEURL + "organizations/?partyrole=Vendor&vendortype=Manufacturer&limit=0&IPADDRESS=" + ipinfo;
    appLoader.CallDataFromServer("GET", getURL, {}).then(function (response) {
        ...
        if (DataDownloadFromServer.length > 0) {
            ...
            var promise1 = new Promise(function (resolve, reject) {
                ...
                DataDownloadFromServer.forEach(function (key, val) {
                    ...
                    if (typeof (_manufacturerName) != 'undefined') {
                        ...
                        ManufacturerdbLoader.ManufacturerListDB.find({ ManufacturerName: _manufacturerName }, function (err, docs) {
                            if (docs.length === 0) {
                                //console.log('INSERT DOC');
                                ManufacturerdbLoader.ManufacturerListDB.insert({ "PartyData": ManufacturerManageArr, "ContactMethod": ContactMethod, "ManufacturerName": _manufacturerName }, function (err) { });
                                tempCount++;
                            } else {
                                if ('VendorID' in docs[0].PartyData) {
                                    ManufacturerdbLoader.ManufacturerListDB.update({ _id: docs[0]._id }, { $set: { "PartyData": ManufacturerManageArr, "ContactMethod": ContactMethod, "ManufacturerName": _manufacturerName } }, function (err) {
                                        if (err) {
                                            errCount += 1;
                                        } else {
                                            console.log('Updated.');
                                    ...
                                    ManufacturerdbLoader.ManufacturerListDB.remove({ _id: docs[0]._id }, function (err, cb) {
                                        if (cb === 1) {
                                            console.log('Updated.');
                                            ManufacturerdbLoader.ManufacturerListDB.insert({ "PartyData": ManufacturerManageArr, "ContactMethod": ContactMethod, "ManufacturerName": _manufacturerName }, function (err) { });
                                            tempCount++;
                            ...
                            if (TotolCount == tempCount) {
                                resolve(TotolCount);
                                DownloadeddbLoader.DownloadedDB.insert({ 'ModuleName': 'Manufacturers', 'Total': TotolCount, 'Downloaded': tempCount });
                            }
            ...
            DownloadeddbLoader.DownloadedDB.insert({ 'ModuleName': 'Manufacturers', 'Total': 0, 'Downloaded': 0 });
