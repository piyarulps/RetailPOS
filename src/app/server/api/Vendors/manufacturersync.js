...
app.get("/", function (req, res) {
    ...
    var FirstTimeDownloadURL = appLoader.BASEURL + "organizations/?partyrole=Vendor&vendortype=Manufacturer&limit=0&IPADDRESS=" + ipinfo;
        ...
        if (DataDownloadFromServer.length > 0) {
            DataDownloadFromServer.forEach(function (key, val) {
                ...
                if (typeof (_manufacturerName) != 'undefined') {
                    ...
                    dbLoader.ManufacturerListDB.find({ 'PartyData.VendorID': key.PartyData.VendorID }, function (err, docs) {
                        if (docs.length === 0) {
                            //console.log('INSERT DOC');
                            dbLoader.ManufacturerListDB.insert({ "PartyData": ManufacturerManageArr, "ContactMethod": ContactMethod, "ManufacturerName": _manufacturerName }, function (err) { });
                        } else {
                            dbLoader.ManufacturerListDB.update({ _id: docs[0]._id }, { $set: { "PartyData": ManufacturerManageArr, "ContactMethod": ContactMethod, "ManufacturerName": _manufacturerName } }, function (err) {
                                if (err) {
                                    errCount += 1;
                                } else {
                                    console.log('Updated.');
    ...
    if (errCount == 0) {
        response = { 'status': 1, 'message': 'Manufacturer sync successfully.' };
    } else {
