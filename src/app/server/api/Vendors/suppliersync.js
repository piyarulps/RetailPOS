...
app.get("/", function (req, res) {
    ...
    var FirstTimeDownloadURL = appLoader.BASEURL + "organizations/?partyrole=Vendor&vendortype=Supplier&limit=0&IPADDRESS=" + ipinfo;
        ...
        if (DataDownloadFromServer.length > 0) {
            DataDownloadFromServer.forEach(function (key, val) {
                ...
                if (typeof (_manufacturerName) != 'undefined') {
                    ...
                    dbLoader.SupplierListDB.find({ 'PartyData.VendorID': key.PartyData.VendorID }, function (err, docs) {
                        if (docs.length === 0) {
                            //console.log('INSERT DOC');
                            dbLoader.SupplierListDB.insert({ "PartyData": ManufacturerManageArr, "ContactMethod": ContactMethod, "SupplierName": _manufacturerName }, function (err) { });
                        } else {
                            dbLoader.SupplierListDB.update({ _id: docs[0]._id }, { $set: { "PartyData": ManufacturerManageArr, "ContactMethod": ContactMethod, "SupplierName": _manufacturerName } }, function (err) {
                                if (err) {
                                    errCount += 1;
                                } else {
                                    //console.log('Updated.');
    ...
    if (errCount == 0) {
        response = { 'status': 1, 'message': 'Supplier sync successfully.' };
    } else {
